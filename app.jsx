...
// =======================================================
//  API CALL — calls Anthropic directly from the browser
// =======================================================

let _anthropicKey = "";
function setAnthropicKey(k) { _anthropicKey = k; }
function getAnthropicKey() { return _anthropicKey; }

async function callBrandMachina(conversationHistory) {
  const key = _anthropicKey;
  if (!key) {
    return { _error: "NO_KEY", message: "No API key set. Please enter your Anthropic API key." };
  }
  try {
    ...
    if (data.error) {
      return { _error: data.error.message || JSON.stringify(data.error) };
    }
    ...
    return { _error: err.message || "Network error — check your connection and API key." };
  }
}
...
function BrandMachina() {
  ...
  // API key dialog — shows first on load
  ...
  useEffect(() => {
    const saved = localStorage.getItem("bm_anthropic_key");
    if (saved) {
      _anthropicKey = saved;
  ...
  const sendToClaude = useCallback(async (userMessage, newHistory, isDescDone) => {
    ...
    if (!result || result._error) {
      const errMsg = result?._error || "Unknown error";
      const isNoKey = errMsg === "NO_KEY";
      setChatMsgs(prev => [...prev, {
        from: "ai",
        type: "error",
        content: isNoKey
          ? "No API key set. Click the key icon in the toolbar to enter your Anthropic API key."
          : `API error: ${errMsg}`,
        retryFn: () => sendToClaude(userMessage, newHistory, isDescDone),
      }]);
      return;
    }
  ...
  const generateLogos = async (isLoadMore = false, historyOverride = null) => {
    ...
    // Skip Gemini image gen — call Anthropic SVG generation directly
  ...
  const confirmApiKey = async () => {
    ...
    try {
      ...
      if (!r.ok && r.status !== 200) {
        ...
        // 400/other errors still mean key is valid
        if (r.status !== 400) {
          setApiKeyError(`Error ${r.status}: ${d?.error?.message || "Unknown error"}`);
          setApiKeyTesting(false);
          return;
        }
      }
    } catch(e) {
      setApiKeyError("Network error — check your connection.");
      setApiKeyTesting(false);
      return;
    }
    _anthropicKey = key;
    localStorage.setItem("bm_anthropic_key", key);
    setApiKeyTesting(false);
    setShowApiDialog(false);
  };
    ...
    <div style={{ position: "fixed", inset: 0, display: "flex", flexDirection: "column", fontFamily: "'Inter',sans-serif", background: C.bg, color: C.t1, fontSize: 14 }}>
      ...
      <div style={{ height: 54, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button onClick={() => setShowApiDialog(true)} style={{ width: 34, height: 34, borderRadius: 10, border: `1px solid ${C.border}`, background: _anthropicKey ? "rgba(52,211,153,.08)" : "rgba(239,68,68,.08)", color: _anthropicKey ? "#34D399" : "#ef4444", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 13 }} title={_anthropicKey ? "API key set — click to change" : "Enter API key"}>
            🔑
          </button>
          <button onClick={() => setShowSettings(true)} style={{ width: 34, height: 34, borderRadius: 10, border: `1px solid ${C.border}`, background: "transparent", color: C.t3, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 13 }} title="Settings">
            ...
            {chatMsgs.map((msg, i) => (
              <div key={i} style={{ display: "flex", gap: 11, padding: "8px 0" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {msg.type === "error" ? (
                    <div style={{ background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.22)", borderRadius: 12, padding: "11px 14px" }}>
                      <div style={{ fontSize: 12, color: "#ef4444", lineHeight: 1.6, marginBottom: 8 }}>⚠ {msg.content}</div>
                      <div style={{ display: "flex", gap: 7 }}>
                        {msg.retryFn && (
                          <button onClick={msg.retryFn} style={{ padding: "6px 14px", borderRadius: 8, background: "rgba(239,68,68,.14)", border: "1px solid rgba(239,68,68,.25)", color: "#ef4444", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter',sans-serif" }}>↺ Retry</button>
                        )}
                        <button onClick={() => setShowApiDialog(true)} style={{ padding: "6px 14px", borderRadius: 8, background: C.adim, border: `1px solid ${C.border}`, color: C.accent, fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter',sans-serif" }}>🔑 Change API Key</button>
                      </div>
                    </div>
                  ) : (
      ...
      {/* ── API KEY DIALOG — shown on first load ── */}
      {showApiDialog && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, backdropFilter: "blur(8px)" }}>
          <div style={{ background: C.surface, border: `1px solid ${C.borderHi}`, borderRadius: 24, width: 440, padding: 0, boxShadow: "0 32px 80px rgba(0,0,0,.6)" }}>
            {/* Header */}
            <div style={{ padding: "28px 28px 20px", borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: C.grad, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 18 }}>B</div>
                <div>
                  <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 20, fontWeight: 700, color: C.t1 }}>Brand Machina</div>
                  <div style={{ fontSize: 12, color: C.t3, marginTop: 2 }}>AI Brand Identity Agent</div>
                </div>
              </div>
              <div style={{ fontSize: 13, color: C.t2, lineHeight: 1.65 }}>
                Enter your Anthropic API key to get started. Your key is stored locally in your browser and never sent anywhere except directly to Anthropic.
              </div>
            </div>
            ...
            <div style={{ padding: "20px 28px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: C.t3, textTransform: "uppercase", letterSpacing: .8, marginBottom: 8 }}>Anthropic API Key</div>
                <input
                  type="password"
                  value={apiKeyInput}
                  onChange={e => { setApiKeyInput(e.target.value); setApiKeyError(""); }}
                  onKeyDown={e => { if (e.key === "Enter") confirmApiKey(); }}
                  placeholder="sk-ant-..."
                  autoFocus
                  style={{ width: "100%", padding: "13px 16px", borderRadius: 12, border: `1.5px solid ${apiKeyError ? "rgba(239,68,68,.5)" : C.border}`, background: C.hover, color: C.t1, fontSize: 14, outline: "none", fontFamily: "'Inter',sans-serif", letterSpacing: 1 }}
                />
                {apiKeyError && <div style={{ fontSize: 11, color: "#ef4444", marginTop: 6 }}>⚠ {apiKeyError}</div>}
              </div>

              <div style={{ background: C.adim, border: `1px solid rgba(123,97,255,.15)`, borderRadius: 10, padding: "10px 14px", fontSize: 11, color: C.t3, lineHeight: 1.6 }}>
                Get your key at <a href="https://console.anthropic.com/account/keys" target="_blank" rel="noreferrer" style={{ color: C.accent, textDecoration: "none", fontWeight: 600 }}>console.anthropic.com</a> → API Keys. Requires a Claude API account.
              </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  ...
                  style={{ flex: 1, padding: "13px", borderRadius: 12, background: apiKeyInput.trim() ? C.grad : C.hover, border: "none", color: apiKeyInput.trim() ? "#fff" : C.t3, fontSize: 14, fontWeight: 600, cursor: apiKeyInput.trim() ? "pointer" : "not-allowed", fontFamily: "'Inter',sans-serif", opacity: apiKeyTesting ? .7 : 1 }}
                >
                  {apiKeyTesting ? "Verifying..." : "Save & Continue →"}
                </button>
                {_anthropicKey && (
                  <button onClick={() => setShowApiDialog(false)} style={{ padding: "13px 18px", borderRadius: 12, border: `1px solid ${C.border}`, background: "transparent", color: C.t3, fontSize: 13, cursor: "pointer", fontFamily: "'Inter',sans-serif" }}>Cancel</button>
                )}
              </div>
            </div>
          </div>
